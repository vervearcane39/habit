...
import { getMockHistoryFromStorage, getStatistics } from '../mock';
...
const ProgressGraph = () => {
      ...
      {historyData.map((day, index) => (
        <div key={day.date} className="flex flex-col items-center flex-1 h-full group">
          <div className="flex-1 flex items-end w-full px-1">
            <div
              className="bg-gradient-to-t from-blue-600 to-blue-400 rounded-t-lg w-full min-h-[8px] transition-all duration-500 ease-out hover:from-blue-700 hover:to-blue-500 relative cursor-pointer shadow-sm"
              style={{
                height: `${Math.max((day.totalScore / maxScore) * 100, 8)}%`
              }}
            >
              {/* Tooltip */}
              <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white px-3 py-1 rounded-md text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10 whitespace-nowrap">
                {day.formattedDate}: {day.totalScore} {day.totalScore === 1 ? 'point' : 'points'}
              </div>
            </div>
          </div>
          
          {/* Date Label */}
          <div className="text-xs text-gray-600 mt-2 text-center transform rotate-0">
            {selectedPeriod === '7' ? 
  ...
  const renderLineChart = () => {
    const points = historyData.map((day, index) => {
      ...
      return { x, y, score: day.totalScore, date: day.formattedDate };
    });
